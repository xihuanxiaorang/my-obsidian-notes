...
update_time: 2025/04/11 23:04
---

## Java

```dataviewjs
const pages = dv.pages("#Resource and #Java")
  .where(p => p.url && p.type)
  .sort(p => p.rate ?? 0, 'desc')
  .sort(p => p.date ?? p.file.ctime, 'desc');

const groups = pages.groupBy(p => p.type);
...
for (let group of groups) {
  ...
  dv.table(
    ["📚 来源", "🔗 链接", "🏷️ 标签", "⭐ 评分", "📅 日期"],
    group.rows.map(p => {
      const linkText = p.type === "📺 Video" ? "▶ 播放" : "📖 跳转";
      const linkEl = dv.el("a", linkText, { href: p.url, target: "_blank" });
...
- 📺视频
	- 教程
		...
		- [Java教程](https://www.bilibili.com/video/BV1dG4y1W7LF?vd_source=84272a2d7f72158b38778819be5bc6ad)
	- Effective Java
