...
tags:
  - MySQL
update_time: 2025/03/24 23:55
create_time: 2025-03-23T22:53:00
---

通用表表达式（Comon Table Expression）能够实现查询结果的重复利用，因此可以用来**简化复杂的子查询和连接查询，提高 SQL 语句的可读性和性能，实现具有<strong style="color:#9bbb59;">层次结构</strong>或者<strong style="color:#9bbb59;">树状结构</strong>数据的遍历**。
...
其中，`WITH` 关键字表示定义通用表表达式（简称 CTE），因此通用表表达式也被称为 `WITH` 查询。t1 和 t2 指定了 CTE 的名称，后面**括号中的内容为可选的字段名**。`AS` 关键字后面的子查询是 CTE 的**定义语句**，定义了它的**表结构**以及**数据**。最后的 `SELECT` 是**主查询语句**，它可以**引用**前面定义的 CTE。除了 `SELECT`，主查询语句也可以是 `INSERT`、`UPDATE` 或 `DELETE` 等。 `WITH` 子句中可以定义多个 CTE，每个 CTE 之间使用逗号分隔，并且<strong style="color:#9bbb59;">已经定义的 CTE 可以被后续的 CTE 引用</strong>。
...
除提高查询的可读性和性能外，CTE 还支持在定义中进行**自我引用**，也就是实现了编程语言中的**递归调用**。递归形式的通用表表达式可以用于遍历具有<strong style="color:#9bbb59;">层次结构</strong>或者<strong style="color:#9bbb59;">树状结构</strong>的数据，例如遍历组织结构、查询地铁换路线图等。其基本语法如下：
...
- cte_query_iterative：表示**递归查询语句**，可以对当前 CTE 进行**自我引用**。<strong style="color:#9bbb59;">每一次递归查询语句执行的结果都会再次作为输入，传递给下一次查询。如果递归查询无法从上一次迭代中返回更多的数据，将会终止递归</strong>。最后，使用 `UNION [ALL]` 运算符合并这些结果集并进行输出。

> [!tip]
>
> 递归 CTE 的执行顺序：
>
> 1. 首先，执行 cte_query_initial 查询语句得到基本结果集 (R<sub>0</sub>)，将该结果集作为第一次递归查询时的输入；
> 2. 然后，执行递归查询语句前获取前一次递归查询之后的结果集 (R<sub>i-1</sub>) 作为输入，然后将本次递归查询之后的结果集 (R<sub>i</sub>) 输出；
...
>
> **递归条件**可以是<strong style="color:#9bbb59;">遍历完表中的所有数据后不再返回更多记录</strong>或者<strong style="color:#9bbb59;">在递归查询部分的 WHERE 子句中指定一个终止条件</strong>。
>
> <strong style="color:#D36B76;">限制递归次数的终止条件必须写在 CTE 的定义中</strong>，写在主查询语句中是没有作用的。
...
员工表 employee 中存储了员工的各种信息，包括员工编号、姓名以及员工经理的编号。该公司的组织结构如下图所示，其中 "刘备" 没有上级，他的经理字段 manager 为空。
<img src="https://fastly.jsdelivr.net/gh/xihuanxiaorang/img/202310260035162.jpeg" style="zoom:50%;" />
...
如果说想对上面的查询限制层级，只查询比 "刘备" 低两级之内的员工信息，也就是不查询 "法正" 的下级员工，该如何实现？定义一个 depth 字段，初始值为 0，每递归一次加一，当 depth >= 2 的时候终止递归即可。
...
而粉丝关系需要使用有向图（Directed Graph）表示。因为关注是单向关联，A 关注了 B，但是 B 不一定关注 A。如下图所示，是一个简单的粉丝关系示意图。<br /><img src="https://fastly.jsdelivr.net/gh/xihuanxiaorang/img/202310260131965.jpeg" style="zoom:50%;" />

如果 A 关注了 B，就会存在一条从 A 到 B 的带箭头的边。上图中的 "刘一" 关注了 "周八"，同时 "刘一" 和 "李四" 相互关注。对于有向图而言，度又分为入度（In-Degree）和出度（Out-Degree）。入度表示有多少条边指向该顶点，出度表示有多少条边是以该顶点为起点的。"刘一" 的入度为 4（微博的粉丝数），出度为 2（微博关注的人数）。
