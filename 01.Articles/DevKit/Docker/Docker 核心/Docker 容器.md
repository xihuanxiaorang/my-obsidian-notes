...
- `OPTIONS`
	...
	- `-v, --volume <宿主机路径>:<容器路径>`：**挂载数据卷**
		- 将宿主机的目录或文件挂载到容器内，支持数据读写共享，实现数据**持久化存储**，防止因容器删除后数据丢失。
		- 路径需为**绝对路径**，即以 `/` 开头。
		- 若宿主机或容器内的目录不存在则会自动创建。
		- 默认以读写模式（`rw`）挂载，如需只读挂载，可在末尾添加 `:ro`，如 `-v /path:/path:ro`。
	- `-d, --detach`：**后台运行容器**
		- **让容器在后台运行，不阻塞当前终端**。
...
- `-v /data/mysql/data:/var/lib/mysql`：挂载数据目录，实现**数据持久化存储**，防止因容器删除后数据丢失。
...
- `-v /data/redis/data:/data`：挂载数据目录，实现**数据持久化存储**，防止因容器删除后数据丢失。
- `-v /data/redis/conf/redis.conf:/etc/redis/redis.conf`：挂载配置文件，支持**自定义参数（如访问密码、监听地址等）**，便于灵活配置。
